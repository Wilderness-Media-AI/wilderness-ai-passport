...
PRIVATE_CONTENT_PATTERNS = {
    ...
    "known private employee name": re.compile(r"\b(?:Jimmy Jiang|Sam Huang)\b", re.IGNORECASE),
    "known device address": re.compile(r"\b4c:11:ae:(?:32:cd:cc|30:e6:40)\b", re.IGNORECASE),
}
...
def check_sensitive_content(files: list[Path], errors: list[str]) -> None:
    for path in files:
        ...
        if path.relative_to(ROOT) != Path("tools/check_repo.py"):
