...
Prefer `./tools/validate.sh --firmware` to verify the complete layout and export
the application image. On provisioned devices, flash that application at
`0x10000` with `tools/safe-flash-app.sh`; use direct `idf.py build/flash` only
for incremental development on unprovisioned test hardware.
