...
> 固件编译优先运行 `./tools/validate.sh --firmware`。门禁会在临时目录验证合并镜像，但只发布
> `build/Wilderness-AI-Passport.bin`；对已有身份的设备，使用
> `tools/safe-flash-app.sh`。`idf.py build` 和
> `idf.py flash` 只作为增量开发命令，不作为默认交付方式。

```bash
source <ESP-IDF-v5.5.3-路径>/export.sh
idf.py --version             # 必须输出 ESP-IDF v5.5.3
./tools/validate.sh --firmware # 优先：编译、验证完整布局并输出应用镜像
...
固件门禁使用全新的临时构建目录，并从仓库 `sdkconfig.defaults` 生成隔离的 `sdkconfig`。它不会读取或覆盖开发者根目录的 `sdkconfig`。门禁会在临时目录校验合并镜像，最终只把应用镜像复制到 `build/Wilderness-AI-Passport.bin` 用于分发，同时强制检查[受保护的 Flash 布局](protected-flash-layout.zh_CN.md)：受保护的 `cardid` 地址、应用大小、分区表 MD5 以及单机身份数据不入包。
...
已有设备只分发 `build/Wilderness-AI-Passport.bin`，并通过 `tools/safe-flash-app.sh`
写入 `0x10000`。合并镜像只保留在本地，用于空白硬件的工厂诊断。
