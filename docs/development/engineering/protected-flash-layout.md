...
- A merged ESP image starting at `0x0`, produced as
  `build/Wilderness-AI-Passport-full.bin`.
...
Release only `build/Wilderness-AI-Passport.bin` for provisioned devices. The validation
gate still checks the temporary merged layout before releasing the application image.

## Flashing safety during development

Never run `idf.py erase-flash` on a provisioned device. It destroys the
per-device identity. Prefer `tools/safe-flash-app.sh`, which writes only the
application image and compares `cardid` before and after. A
