...
在仓库根目录首次编译时，优先运行固件门禁。它会验证临时合并固件，
然后导出用于已配置设备交付的 application-only 镜像：
...
编译方式，它使用隔离的临时构建，验证完整布局，并输出应用镜像：
...
应用镜像只能写入 `0x10000`，不得烧到 `0x0`。保护脚本会备份完整 Flash、写入并校验应用，随后比较刷前刷后的 `cardid`。
