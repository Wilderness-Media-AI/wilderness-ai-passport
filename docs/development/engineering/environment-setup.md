...
verifies a temporary merged image, then exports the application-only image used
for delivery to provisioned devices:
...
isolated temporary build, validates the complete layout, and exports the application image at:

```text
build/Wilderness-AI-Passport.bin
```
...
For a provisioned device, use the protected application-only helper:

```bash
./tools/safe-flash-app.sh --port <port> \
    --app build/Wilderness-AI-Passport.bin
idf.py -p <port> monitor
```

The application belongs at `0x10000`; it must not be written to `0x0`. The helper
backs up the complete Flash, writes and verifies the application, then compares
protected `cardid` before and after.
