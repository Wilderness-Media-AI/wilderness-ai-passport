...
When a delivery action produced an application artifact, verify that exact artifact
on real hardware before treating the project as complete. Download
`Wilderness-AI-Passport.bin`, flash it at `0x10000` with
`tools/safe-flash-app.sh`, and confirm it runs normally. Do not treat a successful
build or upload as hardware validation. If it does not run, stop and fix before
closing out. See
...
3. Let the tagged build produce the application-only firmware `.bin`.
...
6. Verify the released application build on hardware (see
...
- For day-to-day compilation prefer `idf.py build` (fast, incremental). Before
  a release or delivery, use `./tools/validate.sh --firmware` to verify the
  complete layout and export the application-only image. Flash that artifact
  at `0x10000` with `tools/safe-flash-app.sh` on provisioned devices.
