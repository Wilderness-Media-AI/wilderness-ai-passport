...
交付动作产出应用镜像后，在把项目视为完成前先用真机验证该文件。下载 `Wilderness-AI-Passport.bin`，通过 `tools/safe-flash-app.sh` 写入 `0x10000` 并确认正常运行。不要把构建或上传成功当作硬件验证。若不能运行，先停下修复，再继续收口。产物与烧录见 [`CI-build-and-release.md`](../ci/CI-build-and-release.zh_CN.md)。
...
3. 让 tagged 构建产生 application-only 固件 `.bin`。
...
6. 在真机验证发布的应用构建（见 [发布后真机验证](#post-release-hardware-verification)）。
...
- 日常编译优先用 `idf.py build`（快、增量）。发布或交付前使用
  `./tools/validate.sh --firmware` 验证完整布局并导出 application-only
  镜像。对已配置设备，通过 `tools/safe-flash-app.sh` 把该产物写入
  `0x10000`。
