...
Prefer the repository firmware gate for builds. It verifies a merged image
internally but exports only the application image for distribution. On a
provisioned device, flash that application at `0x10000` with
`tools/safe-flash-app.sh`. The direct IDF commands below are for incremental
development on unprovisioned test hardware.

```bash
source <path-to-esp-idf-v5.5.3>/export.sh
idf.py --version             # must report ESP-IDF v5.5.3
./tools/validate.sh --firmware # verifies layout; exports application image
